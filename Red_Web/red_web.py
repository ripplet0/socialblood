...
def accepted_users(p):
    db=Db()
    db.update("update login set usertype='user' where login_Id='"+str(p)+"'")

    return '<script>alert("accepted");window.location="/acceptReject_users"</script>'
...
def rejected_donor(p):
    ...
    return '<script>alert("rejected");window.location="/acceptReject_users"</script>'
